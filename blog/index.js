...
app.get("/callback", function(req, res, next) {
  ...
  if (state === null || state !== auth_state) {
    next(new Error("The state doesn't match")); // El estado no coincide
  }

  res.clearCookie("auth_state"); // limpiar cookie
  ...
  // Se hace peticion internamente
  request.post(authOptions, function(error, response, body) {
    if (error || response.statusCode !== 200) {
      next(new Error("The token is invalid"));
    }

    // httpOnly => se lea solo del lado del servidor
